...
class Gui_MainWindow(Qw.QMainWindow):
	...
	def file_open_command(self, parent_dir, path, isdir, fm):
		dir_cmd = {'dolphin': ['dolphin', '--select', path], 'nemo': ['nemo', parent_dir],
				   'nautilus': ['nautilus', parent_dir], 'doublecmd': ['doublecmd', parent_dir]}
		file_cmd = {'dolphin': ['dolphin', '--select', path], 'nemo': ['nemo', path], 'nautilus': ['nautilus', path],
					'doublecmd': ['doublecmd', path]}
		if isdir:
			if fm in dir_cmd.keys():
				return dir_cmd[fm]
			else:
				return ['xdg-open', path]
		else:
			if fm in file_cmd.keys():
				return file_cmd[fm]
			else:
				return ['xdg-open', parent_dir]
	...
	def double_click_enter(self, QModelIndex, from_enter=False, shift=False):
		...
		if fm == 'startfile':
			...
			if not bool(set(fm) & set('thunar', 'pcmanfm', 'spacefm')):
				subprocess.Popen(self.file_open_command(parent_dir, path, is_dir, fm))
			else:
				if 'thunar' in fm:
					...
					subprocess.Popen([fm, parent_dir])
